...
$backendStdout = Join-Path $logsDir "backend.mysql.stdout.log"
$backendStderr = Join-Path $logsDir "backend.mysql.stderr.log"
$frontendStdout = Join-Path $logsDir "frontend.mysql.stdout.log"
$frontendStderr = Join-Path $logsDir "frontend.mysql.stderr.log"
$stateFile = Join-Path $logsDir "start-mysql.state.json"
...
$backendProc = Start-Process `
    ...
    -ArgumentList @("-ExecutionPolicy", "Bypass", "-File", $backendStartScript, "-Detached", "-UseMySql") `
...
if ($backendReady) {
    Write-Host "Backend: http://localhost:8080 (MySQL profile)"
} else {
